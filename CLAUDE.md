...
| File | Change | Why |
|---|---|---|
| `.github/CODEOWNERS` | Fork-local block appended at EOF, one line per fork module (`/modules/waifu_test/`, `/modules/projectiles/`, `/modules/box3d_physics/`, all `@Th3D0c0`, plus `/modules/limboai` without a trailing slash) | Required by the `validate-codeowners` CI hook; every file under `modules/` must have an owner. Placed at EOF (last-match-wins) so it also claims each module's `SCsub`/`config.py`, and because appending conflicts less than inserting into the Modules section. Add a line here with every new module. |
| `COPYRIGHT.txt` | Five-line entry for `thirdparty/box3d/` (MIT, Erin Catto) | Godot lists every vendored library's license here, and the fork inherits that convention. Additive, on the `feature/box3d-physics` branch only. |
| `thirdparty/README.md` | Fourteen-line section describing the Box3D vendoring — version `0.1.0` (`8441b4a`), what was copied and what was left out | Same convention: the file is the index of what lives under `thirdparty/`. Additive, on the `feature/box3d-physics` branch only. |
| `pyproject.toml` | Three lines excluding `thirdparty/box3d/` from the Python tooling globs | Keeps `ruff` and friends off vendored C sources. Additive, on the `feature/box3d-physics` branch only. |
