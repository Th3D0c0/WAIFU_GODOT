...
| `.github/changed_files.yml` | `!modules/limboai/**` added to the `clangd` filter | Follows from the `static_checks.yml` row: once that job clones submodules, `changed-files` recurses into limboai and any bump of the pin feeds clangd-tidy vendored source. It flagged `unused-includes` on `compat_window_wrapper.h` (a false positive — `WindowWrapper` is used only through a `#define`) and on `test_set_var.h`. Sits next to the existing `!**/thirdparty/**`, which is what a vendored submodule morally is. Only bites on pushes that move the gitlink, so it will look dormant. |
